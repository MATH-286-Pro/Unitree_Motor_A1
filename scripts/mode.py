...
# 添加全局变量以存储位置调整 
pos_adjustment = 0.0
pos_increment = 0.1  # 你可以根据需要调整这个值
...
def send_commands():
    while not exit_event.is_set():  # 检查退出事件是否被设置
        ...
        # 开启 ? 模式
        c.send_recv(fd, byref(motor_t1_start), byref(motor_r1))  
...
keyboard.on_press_key("up", increase_position)
keyboard.on_press_key("down", decrease_position)
